...
import { ArrowRight, BarChart, ChevronDown, Home, PieChart } from "lucide-react";
import Image from "next/image";
import { ReactNode, useEffect, useRef, useState } from "react";

export default function ShiftingDropDown() {
  return (
    <div className="flex h-96 pt-6 justify-start bg-neutral-950 text-neutral-200 md:justify-center">
      <Tabs />
    </div>
  );
}
...
const Tab = ({
  ...
  return (
    <button
      ...
      className={
        `group flex items-center gap-1 rounded-full px-3 py-1.5 text-sm transition-colors
        ${selected === tab
...
const Content = ({ dir, selected }: { dir: "l" | "r" | null, selected: number | null }) => {
  return (
    <motion.div
      id="overlay-content"
      initial={{
...
const Bridge = () => {
  return (
    <div className="absolute -top-[28px] left-0 right-0 h-[32px]" />
  );
}
